...
autoplot(tm) + ggtitle("set and get 10 times")
```

Note: `collection::OrderedDictL` uses R environments and may leak memory. It
is not recommended though it is marginally faster than
`collection::OrderedDict` when there are only small amount of items.

```{r ordered_dict2, echo = FALSE}
tm <- microbenchmark(
