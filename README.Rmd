...
Dictionary. Comparing to R environments, `dict()` does not [leak memory](https://r-lib.github.io/fastmap/#memory-leak-examples) and supports various other types of keys.
```{r}
d <- dict()
e <- new.env()
